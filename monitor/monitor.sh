...
check_timer(){
  ...
  if [ `echo ${timer_pid} | wc -w` -eq 0 ];then
    ...
    # gunicorn --timeout 600 -w 4 -c gunicorn_config.py m3u8ProxyServer:app >> /dev/null 2>&1 &
    cd ${project_path}
    gunicorn --timeout 600 -w 4 -c gunicorn_config.py m3u8ProxyServer:app >> ${project_path}/run.log 2>&1 &
    get_pid

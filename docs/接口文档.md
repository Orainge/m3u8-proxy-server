...
|» ENP|body|boolean| 否 | 是否启用代理|（仅代理非MPD类型时生效）|
|» COK|body|object| 否 | 请求携带的 Cookie|（仅代理非MPD类型时生效）请求 URL 时，自动携带对应的 Cookie 键值对，例如：{key: value}|
|» MMS|body|boolean| 否 | 是否只保留最清楚的 M3U8 视频流|（仅代理M3U8类型时生效）|

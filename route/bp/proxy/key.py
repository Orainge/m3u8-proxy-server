...
from route.consts.uri_param_name import URI_NAME_KEY
from route.service import m3u8 as m3u8_proxy_service
from util import encrypt as encrypt_util

# 创建蓝图
proxy_key_bp = Blueprint("proxy_key", __name__, url_prefix=f'/{URI_NAME_KEY}')


# 代理接口：请求视频文件
@proxy_key_bp.route('/<encrypt_url>', methods=['GET'])
def proxy_key_file(encrypt_url):
    # 解密 URL
    try:
    ...
    # 获得响应
    response = m3u8_proxy_service.proxy_key(url, enable_proxy)
